...
function buildPreRoundCall(round: TrioRound): SpeechLine[] {
  const situation = round.cards[0]?.moment.situation ?? "";
  ...
  const names = `${a}, ${b}, and ${c}`;
  return [
    { text: pick("open", OPENERS), rate: 1.08, pitch: 1.0 },
    { text: pick("scene", SCENES)(situation), rate: 1.02, pitch: 0.9 },
    { text: pick("trio", TRIO_LINES)(names, verb), rate: 1.05, pitch: 0.95 },
    { text: pick("handoff", HANDOFFS), rate: 1.08, pitch: 1.0 },
  ];
}
...
function windupLine(c: TrioCard): string {
  ...
  const pool = c.role.startsWith("IN THE CIRCLE")
    ...
    : c.role.startsWith("IN THE FIELD")
      ? WINDUPS_FIELD
      : WINDUPS_PLATE;
  return pick("windup", pool)(first);
}

// Keyed by action substrings, same order classify() checks them.
function outcomeLine(c: TrioCard): string {
...
function buildOutcomeCall(card: TrioCard, correct: boolean): SpeechLine[] {
  return [
    { text: windupLine(card), rate: 1.0, pitch: 0.88 },
    { text: outcomeLine(card), rate: 1.15, pitch: 1.0, atMs: FLASH_MS },
    { text: pick("verdict", correct ? VERDICTS_HIT : VERDICTS_MISS), rate: 1.05, pitch: 0.92, delayMsBefore: 250 },
  ];
}
...
function buildReplayCall(card: TrioCard): SpeechLine[] {
  return [
    { text: pick("replay", REPLAY_OPENERS), rate: 1.08, pitch: 1.0 },
    { text: outcomeLine(card), rate: 1.15, pitch: 1.0, atMs: FLASH_MS },
  ];
}
