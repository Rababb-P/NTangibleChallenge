// iOS 26 "Liquid Glass" UI kit — ported verbatim (TS) from the recovered app's
// iOS.jsx. Exact bezel, status bar, glass nav pills, grouped inset lists, and
// keyboard. Extended with light interaction props (onBack / trailing / leading)
// for real navigation while preserving the exact look.
...
export function IOSGlassPill({ children, dark = false, style = {}, onClick }: { children: ReactNode; dark?: boolean; style?: CSSProperties; onClick?: () => void }) {
  return (
    <div onClick={onClick} className={onClick ? "nt-tap" : undefined} style={{ height: 44, minWidth: 44, borderRadius: 9999, position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: dark ? "0 2px 6px rgba(0,0,0,0.35), 0 6px 16px rgba(0,0,0,0.2)" : "0 1px 3px rgba(0,0,0,0.07), 0 3px 10px rgba(0,0,0,0.06)", ...style }}>
...
export function IOSNavBar({ title = "Title", dark = false, onBack, trailing }: { title?: string; dark?: boolean; onBack?: () => void; trailing?: ReactNode }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingTop: 62, paddingBottom: 10, position: "relative", zIndex: 5 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px" }}>
        {onBack ? (
          <IOSGlassPill dark={dark} onClick={onBack}>
...
export function IOSListRow({ title, detail, icon, chevron = true, isLast = false, dark = false, onClick }: { title: ReactNode; detail?: ReactNode; icon?: string; chevron?: boolean; isLast?: boolean; dark?: boolean; onClick?: () => void }) {
  const text = dark ? "#fff" : "#000";
  const sec = dark ? "rgba(235,235,245,0.6)" : "rgba(60,60,67,0.6)";
  const ter = dark ? "rgba(235,235,245,0.3)" : "rgba(60,60,67,0.3)";
  const sep = dark ? "rgba(84,84,88,0.65)" : "rgba(60,60,67,0.12)";
  return (
    <div onClick={onClick} className={onClick ? "nt-tap" : undefined} style={{ display: "flex", alignItems: "center", minHeight: 52, padding: "0 16px", position: "relative", fontFamily: "-apple-system, system-ui", fontSize: 17, letterSpacing: -0.43 }}>
...
export function IOSDevice({ children, width = 402, height = 874, dark = false, title, onBack, trailing, navBar = true, keyboard = false }: { children: ReactNode; width?: number; height?: number; dark?: boolean; title?: string; onBack?: () => void; trailing?: ReactNode; navBar?: boolean; keyboard?: boolean }) {
  return (
    <div style={{ width, height, borderRadius: 48, overflow: "hidden", position: "relative", background: dark ? "#000" : "#F2F2F7", boxShadow: "0 40px 80px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.12)", fontFamily: "-apple-system, system-ui, sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
        {navBar && title !== undefined && <IOSNavBar title={title} dark={dark} onBack={onBack} trailing={trailing} />}
        <div className="nt-scroll" style={{ flex: 1, overflow: "auto" }}>{children}</div>
...
export function IOSKeyboard({ dark = false }: { dark?: boolean }) {
  ...
  const key = (content: ReactNode, opts: { w?: number; flex?: boolean; ret?: boolean; fs?: number; k?: string } = {}) => {
